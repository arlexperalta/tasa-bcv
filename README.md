...
Y esa segunda rama no se va vacía. **Las impresiones son la primera cuenta que existe de la base recurrente de tasa:** la pieza solo se le muestra a quien ya había vuelto, así que el total en seis semanas mide algo que nadie midió nunca — las ~3.000 sesiones al mes jamás se separaron en "gente que vuelve" y "gente de paso".

Ese número tiene tres partes y las tres hacen falta para leerlo: son **dispositivos y no personas** (dos aparatos cuentan dos veces); es un **piso y no un total** (no aparece quien borró los datos del navegador ni quien la tuvo en segundo plano); y es gente que volvió **y además usó la calculadora hasta el final** —monto en los dos campos y tres segundos quieta mirando el resultado—, porque esa es la condición que dispara la pieza.

La tercera es la que lo vuelve valioso, y por eso **no es comparable con las sesiones de Umami**: quien las cruce va a concluir que la pieza pierde gente, y no pierde nada — descarta al que solo le echó un ojo al número del día y se fue. No son visitantes recurrentes, son **usuarios recurrentes**. El script no le pone umbral automático a propósito: inventar una vara de audiencia sin con qué fundamentarla sería el mismo error que la rama viene a corregir.
